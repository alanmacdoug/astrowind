...
title: "The SPF Generator Walkthrough: Build Your Record One Answer at a Time"
...
The tool walks you through a series of questions about your sending setup. Each answer confirms a sender — Google Workspace, Microsoft 365, a marketing platform, custom servers — and adds the matching mechanism to your record. As you go, two things accumulate: the record itself, and the lookup budget it consumes.

The record assembles at the bottom of the page as you answer. Nothing is sent anywhere, nothing is stored — the tool runs entirely in your browser.
...
**Does Microsoft 365 send email as your domain?** Similar mechanics, similar appetite. Note that Microsoft's include lives under `spf.protection.outlook.com`, and it is tenant-specific in effect even though the include string is shared — the record works because Microsoft's servers handle the tenant matching during evaluation.
...
**Do you have custom servers sending as your domain?** For most readers, no. If yes, this is the only place raw IP addresses belong in your record, via `ip4` or `ip6` mechanisms — entered deliberately, as few as you can manage.

**Are there any other services?** The catch-all, and deliberately last. Order receipts from your invoicing software, website contact forms, CRM notifications — the senders nobody remembers until the record is already published. A common pattern this question catches: a business authenticates its newsletter and its staff mail, forgets the booking system, and then wonders why booking confirmations are the one message type landing in spam.
...
- **One SPF record per domain.** If a TXT record already exists starting `v=spf1`, you edit it — you do not add a second. Two SPF records means neither is evaluated; receivers are required to treat the situation as an error.
...
Honest boundaries. The generator is built for the common case: a small business with a handful of known senders assembling a first sensible record. It is less suited to untangling an existing record you inherited — flattened includes, nested redirects, third-party senders nobody remembers commissioning. That is an audit-shaped problem, not a wizard-shaped one.
